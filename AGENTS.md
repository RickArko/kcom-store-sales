...
- **Package** `src/store_sales/`: `data.py` (loaders + merges), `features.py` (`TimeSeriesFeatureEngineer` with lags/rolling/date/Fourier features), `models.py` (`TimeSeriesModel`), `metrics.py` (wraps `kaggle_ml.evaluation.rmsle`), `recursive.py` (`recursive_forecast` + `onehot_fit`/`onehot_align` for recursive multi-step linear forecasting), `tracking.py` (re-exports `kaggle_ml.tracking`), `nixtla_pipeline.py` (Nixtla long-format stats/ML baselines), `viz.py` (animated pipeline GIF).
...
- **Linear data flow**: `merge_tables()` → `apply_preprocessing()` → `create_lag_features()` (log_sales lags + rolling + Fourier) → `timeseries_split()` → `onehot_fit()` → `Ridge.fit()` → `recursive_forecast()` predicts val + test day-by-day (each day's prediction feeds back as lag for next day). `transactions` is dropped (unknown for the 16-day test horizon). Val RMSLE: 0.51.
